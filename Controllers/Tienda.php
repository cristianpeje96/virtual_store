<?php 
	...
	require_once("Models/TSubcategoria.php");
	...
	class Tienda extends Controllers{
		public $model;
		public $views;
		use TCategoria, TSubcategoria, TProducto, TCliente;
		public $login;
		...
		public function tienda()
		{
			...
			$data['subcategorias'] = $this->getSubcategorias();
			$data['talla'] = $this->getTallas();
			$data['colores'] = $this->getColores();
		...
		public function subcategoria($params){
			if(empty($params)){
				header("Location:".base_url());
			}else{
				$arrParams = explode(",",$params);
				$idsubcategoria = intval($arrParams[0]);
				$ruta = strClean($arrParams[1]);
				$pagina = 1;
		
				// Verificar si hay más parámetros para la paginación
				if(count($arrParams) > 1 AND is_numeric($arrParams[1])){
					$pagina = $arrParams[1];
				}
		
				// Obtener el total de productos en la subcategoría
				$cantProductos = $this->cantProductos($idsubcategoria);
				$total_registro = $cantProductos['total_registro'];
				$desde = ($pagina-1) * PRODSUBCATEGORIA; // Límite por página definido en una constante
				$total_paginas = ceil($total_registro / PRODSUBCATEGORIA);
		
				// Obtener los productos filtrados por subcategoría
				$infoSubcategoria = $this->getProductosSubcategoriaT($idsubcategoria, $desde, PRODSUBCATEGORIA);

				$data['page_tag'] = NOMBRE_EMPESA." - ".$infoSubcategoria['subcategoria'];
				$data['page_title'] = $infoSubcategoria['subcategoria'];
				$data['page_name'] = "subcategoria";
				$data['productos'] = $infoSubcategoria['productos'];
				$data['infoSubcategoria'] = $infoSubcategoria;
				$data['pagina'] = $pagina;
				$data['total_paginas'] = $total_paginas;
				$data['subcategorias'] = $this->getSubcategorias(); // Para mostrar las subcategorías disponibles
				...
				if(empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmailCliente']))
				{
					...
					if($request_user > 0 )
					{
						...
						$dataUsuario = array('nombreUsuario' => $nombreUsuario,
											 ...
											 'asunto' => 'Bienvenido a Pipe shoes');
				...
				if(!empty($_SESSION['arrCarrito'])){
					...
					if(empty($_POST['datapay'])){
						...
						if($request_pedido > 0 ){
							...
							$userEmail = isset($_SESSION['userData']['email_user']) ? $_SESSION['userData']['email_user'] : 'default@example.com';
							...
							$orden = !empty($request_pedido) ? openssl_encrypt($request_pedido, METHODENCRIPT, KEY) : '';
							$transaccion = !empty($idtransaccionpaypal) ? openssl_encrypt($idtransaccionpaypal, METHODENCRIPT, KEY) : '';
							...
							// Limpia cualquier salida en buffer antes de regenerar la sesión

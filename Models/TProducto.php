...
trait TProducto{
	...
	public function getProductosSubcategoriaT(int $idsubcategoria, string $ruta, $desde = null, $porpagina = null){
		...
		$sql_subcat = "SELECT idsubcategoria, nombre, ruta FROM subcategoria WHERE idsubcategoria = '{$this->intIdSubcategoria}'";
		$request = $this->con->select($sql_subcat);
	
		if(!empty($request)){
			$this->strSubcategoria = $request['nombre'];
			$this->strRutaSubcategoria = $request['ruta'];
	
			// Consultar productos que pertenecen a la subcategoría
			$sql = "SELECT p.idproducto,
							p.codigo,
							p.nombre,
							p.descripcion,
					...
					ON p.subcategoriaid = s.idsubcategoria
					WHERE p.status != 0 AND p.subcategoriaid = $this->intIdSubcategoria AND s.ruta = '{$this->strRuta}'
					ORDER BY p.idproducto DESC ".$where;
			...
			$request = array(
				'idsubcategoria' => $this->intIdSubcategoria,
				'ruta' => $this->strRutaSubcategoria,
				'subcategoria' => $this->strSubcategoria,
				'productos' => $request
			);
		}
	...
	public function getProductoT(int $idproducto, string $ruta){
		...
		$sql = "SELECT 
			    ...
			    p.ruta,
			    p.stock,
			    co.nombrecolor AS colores,
			    co.codigo_hex AS codigo_hex,
			    t.talla AS talla
				FROM 
				    producto p
				INNER JOIN 
				    categoria c ON p.categoriaid = c.idcategoria
				LEFT JOIN 
				    colores co ON p.colorid = co.idcolor
				LEFT JOIN 
				    producto_talla pt ON p.idproducto = pt.idproducto
				LEFT JOIN 
				    talla t ON pt.tallaid = t.idtalla
				WHERE 
			    p.status != 0 
